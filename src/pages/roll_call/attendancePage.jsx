...
const AttendancePage = () => {
  ...
  useEffect(() => {
    const fetchTodaysSchedule = async () => {
      ...
      try {
        ...
        const response = await fetch(`https://sparlex-spa.up.railway.app/api/v1/users-schedules/user/${userId}/schedule`, {
  ...
  const handleCheckIn = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://sparlex-spa.up.railway.app/api/v1/users-schedules/check-in/${todaysSchedule.id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleCheckOut = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://sparlex-spa.up.railway.app/api/v1/users-schedules/check-out/${todaysSchedule.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
